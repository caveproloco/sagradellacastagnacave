(function(){
  ...
  function wireButtons(){
    ...
    btnAccept?.addEventListener('click', ()=>{
      setConsent({ necessary:true, thirdparty:true });
      // abilita GA solo dopo consenso
      window.enableAnalyticsConsent && window.enableAnalyticsConsent();
      b.hidden = true;
      applyConsent();
    });
    ...
    btnReject?.addEventListener('click', ()=>{
      setConsent({ necessary:true, thirdparty:false });
      b.hidden = true;
      applyConsent();
    });
    ...
    btnSave?.addEventListener('click', ()=>{
      ...
      if(form){
        prefs.thirdparty = !!form.querySelector('input[name="thirdparty"]')?.checked;
      }
      setConsent(prefs);

      // abilita GA solo se l’utente ha dato consenso alla categoria scelta
      if (prefs.thirdparty) {
        window.enableAnalyticsConsent && window.enableAnalyticsConsent();
      }
    ...
    // Sblocco per embed on-demand (es. pulsante "Abilita mappa")
