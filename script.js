...
/* =========================================================================
   ...
   ======================================================================== */
const EVENT_START_ISO = '2026-10-23T20:00:00+02:00';
...
/* =========================================================================
   Bottone "Torna su" — auto-inject su tutte le pagine
   ======================================================================== */
(function initToTop(){
  const TH = 400; // soglia in px per mostrare il bottone
    ...
    if (!btn) {
      ...
      btn.innerHTML = `
        <svg viewBox="0 0 24 24" width="22" height="22" aria-hidden="true">
          <path fill="currentColor" d="M12 6a1 1 0 0 1 .7.3l6 6a1 1 0 1 1-1.4 1.4L12 8.41l-5.3 5.3a1 1 0 1 1-1.4-1.41l6-6A1 1 0 0 1 12 6z"/>
        </svg>`;
  ...
  function attach(){
    ...
    // click: scroll a inizio pagina anche se non esiste #top
    btn.addEventListener('click', (e) => {
      e.preventDefault();
      window.scrollTo({ top: 0, behavior: 'smooth' });
    });

    const toggle = () => btn.classList.toggle('show', window.scrollY > TH);
    toggle();
...
// ===== Autoplay robusto: ritenta su viewport + primo gesto utente + fallback controlli =====
(function () {
  const v = document.getElementById('heroVideo');
  if (!v) return;
  ...
  const tryPlay = () => {
    ...
    if (p && typeof p.then === 'function') {
      p.catch(() => { /* ignoriamo: ritentiamo al gesto utente */ });
    }
  };

  // primo tentativo appena pronto
  if (v.readyState >= 2) tryPlay();
  else v.addEventListener('canplay', tryPlay, { once: true });

  // ritenta quando entra davvero in viewport (se hai già un IO sopra, non crea conflitto)
  ...
  // fallback: primo gesto utente (tap/click/scroll/tasto) => riprova
  const userEvents = ['touchstart','click','scroll','keydown'];
  const onUserGesture = () => { tryPlay(); cleanup(); };
  const cleanup = () => userEvents.forEach(ev => window.removeEventListener(ev, onUserGesture));
  userEvents.forEach(ev => window.addEventListener(ev, onUserGesture, { passive: true }));

  // se dopo 2s è ancora fermo, mostra i controlli
  setTimeout(() => {
...
/* Evidenzia la voce di menu in base alla pagina .html */
(function highlightActivePage(){
  const links = document.querySelectorAll('.mainnav a[href$=".html"], .mobile-nav a[href$=".html"]');
  if (!links.length) return;
  const here = (location.pathname.split('/').pop() || 'index.html').toLowerCase();
  links.forEach(a => {
    const href = (a.getAttribute('href') || '').toLowerCase();
    const isActive = href.endsWith(here);
...
/* =========================================================================
   FILMSTRIP (5 per vista desktop, 2 mobile) – loop e frecce
   ======================================================================== */
(function initFilmstrips(){
  const roots = document.querySelectorAll('[data-filmstrip]');
  ...
  roots.forEach(root => {
    ...
    // cloni per loop
    const perDesktop = 5, perMobile = 2;
    const mqMobile = window.matchMedia('(max-width:780px)');
    const perView = () => (mqMobile.matches ? perMobile : perDesktop);

    const cloneCount = () => Math.min(perView(), slides.length);
    function buildClones(){
      // ripulisci cloni eventuali (ri-costruzione su resize)
      track.querySelectorAll('.__clone').forEach(c => c.remove());
      const head = slides.slice(0, cloneCount()).map(s => s.cloneNode(true));
      const tail = slides.slice(-cloneCount()).map(s => s.cloneNode(true));
      head.forEach(c => { c.classList.add('__clone'); track.appendChild(c); });
      tail.forEach(c => { c.classList.add('__clone'); track.insertBefore(c, track.firstChild); });
      slides = Array.from(track.children);
    }
    ...
    let index = 0;                    // relativo al primo originale
    let offset = 0;                   // numero di cloni in testa
    let anim = false;

    function layout({instant=false} = {}){
      const w = basis() + gap();
      track.style.transition = instant ? 'none' : 'transform .3s ease';
      track.style.transform = `translateX(${- (index + offset) * w}px)`;
    }

    function normalize(){
      const originals = slides.length - 2*cloneCount();
      if (index < 0){ index += originals; layout({instant:true}); }
      else if (index >= originals){ index -= originals; layout({instant:true}); }
      anim = false;
    }

    function next(){ if (anim) return; anim = true; index++; layout(); }
    function prev(){ if (anim) return; anim = true; index--; layout(); }
    ...
    // drag/swipe
    let dragging=false, sx=0, startTX=0;
    const curTX = () => {
      const m = track.style.transform.match(/translateX\\((-?\\d+(?:\\.\\d+)?)px\\)/);
      return m ? parseFloat(m[1]) : 0;
    };
    function startDrag(e){
      dragging = true; anim=false;
      sx = (e.touches ? e.touches[0].pageX : e.pageX);
      startTX = curTX();
      track.style.transition = 'none';
      e.preventDefault();
    }
    function moveDrag(e){
      if (!dragging) return;
      const x = (e.touches ? e.touches[0].pageX : e.pageX);
      track.style.transform = `translateX(${startTX + (x - sx)}px)`;
    }
    function endDrag(){
      if (!dragging) return; dragging=false;
      const w = basis() + gap();
      const iFloat = -curTX() / w;
      index = Math.round(iFloat - offset);
      layout();  // poi normalize
    }
...
// Lightbox per .g-item (gallery)
(function initGalleryLightbox(){
  const lb = document.getElementById('lightbox');
  if (!lb) return; // esegui solo in gallery

  const links    = Array.from(document.querySelectorAll('.g-item'));
  if (!links.length) return;
  ...
  function openAt(index){
    ...
    if (counter) counter.textContent = `${i+1} / ${links.length}`;
    lb.hidden = false;
  ...
  const next = () => openAt(i+1);
  const prev = () => openAt(i-1);
  ...
  img.addEventListener('load', () => {
    const aNext = links[(i+1) % links.length];
    const pre = new Image();
    pre.src = aNext.getAttribute('href');
  });
})();
